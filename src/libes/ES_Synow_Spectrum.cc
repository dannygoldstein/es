...
std::vector<ES::Synow::icoord> ES::Synow::Spectrum::in ( const ES::Synow::Setup& setup ){
  //this->operator()( setup );
  std::vector<ES::Synow::icoord> vec;
  for (int ip = 0; ip < _p_total; ip++){
    double v = setup.v_phot * _p[ ip ];
    ES::Spectrum s = ES::Spectrum::create_from_spectrum( *_output );
    for (int iw = 0; iw < s.size(); iw++)
      s.flux( iw ) = _in[ ip ][ iw ];
    ES::Synow::icoord coord;
    coord.v = v;
    coord.s = s;
    vec.push_back( coord );
  }
